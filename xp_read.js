...
let ReadArr = [], XpBody = "",readscore = 0;
  if (process.env.XP_READ && process.env.XP_READ.indexOf('&') > -1) {
  XpBody = process.env.XP_READ.split('&');
  console.log(`您选择的是用"&"隔开\n`)
  }
  else if (process.env.XP_READ && process.env.XP_READ.indexOf('\n') > -1) {
  XpBody = process.env.XP_READ.split('\n');
  console.log(`您选择的是用换行隔开\n`)
  } else {
  XpBody = process.env.XP_READ.split()
  }
  Object.keys(XpBody).forEach((item) => {
        if (XpBody[item]) {
          ReadArr.push(XpBody[item])
        }
    })
    ...
    return new Promise((resolve, reject) => {
        ...
        $.post(url, async(error, response, data) => {
           let readres = JSON.parse(data);
             //console.log(data)
           if (readres.resultCode == 1 && typeof readres.data.goldCoinNumber === 'number') {
              console.log(`\n本次阅读获得${readres.data.goldCoinNumber}金币💰，请等待30s后执行下一次阅读\n`);
